...
| 系統類別 | 優化項目 | 升級說明 |
|----------|----------|----------|
| **穩定性** | 瀏覽器編碼修正 | 自動為上傳的 SRT/JSON 設定 `charset=utf-8`，徹底解決新分頁開啟出現亂碼的問題。 |
| **體驗優化** | UI 介面簡化 | 移除冗餘的右下角懸浮按鈕，並統一對話視窗 ID，修復對話框初始化與輸入衝突。 |
| **預覽模式** | TEST 模式增強 | `TEST.json` 更名為 `preview.json` 並支援模擬 AI 對話回應，達成「無後端也能展示完整流程」。 |
| **監控準確度** | 雲端/地端監控分離 | 修復雲端版 HTML 誤監聽本地引擎的問題，現已正確分別監控 `local_engine` 與 `cloud_engine`。 |
| **工具擴充** | `scratch/` 工具組 | 新增 `fix_all_metadata.py` 等腳本，方便開發者快速修正雲端檔案的 MetaData。 |
